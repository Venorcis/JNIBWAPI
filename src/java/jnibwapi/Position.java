...
/**
 * Generalised representation of a position for JNIBWAPI. Instead of breaking
 * positions into pixel, walk tile, and build tile positions, as in BWAPI, this
 * class represents all position types and converts between them when required.
 * Position objects are immutable. They should be compared using their equals()
 * method or by comparing the x and y values at the desired granularity.
 */
public class Position {
	...
	public int getY(PosType posType) {
		return this.y / posType.scale;
	}

	/** Returns the x-coordinate, in pixels */
	public int getPX() {
		return this.x / PosType.PIXEL.scale;
	}

	/** Returns the y-coordinate, in pixels */
	public int getPY() {
		return this.y / PosType.PIXEL.scale;
	}

	/** Returns the x-coordinate, in walk tiles */
	public int getWX() {
		return this.x / PosType.WALK.scale;
	}

	/** Returns the y-coordinate, in walk tiles */
	public int getWY() {
		return this.y / PosType.WALK.scale;
	}

	/** Returns the x-coordinate, in build tiles */
	public int getBX() {
		return this.x / PosType.BUILD.scale;
	}

	/** Returns the y-coordinate, in build tiles */
	public int getBY() {
		return this.y / PosType.BUILD.scale;
	}
	...
	public int getApproxPDistance(Position target) {
		int min = Math.abs(this.x - target.x);
		int max = Math.abs(this.y - target.y);
		if (max < min) {
			int temp = max;
			max = min;
			min = temp;
	...
	public boolean isValid() {
		...
		if (map == null) {
			...
			return this.x < map.getSize().getPX() && this.y < map.getSize().getPY();
		}
	}
